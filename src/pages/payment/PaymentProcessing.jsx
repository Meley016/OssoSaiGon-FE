      ...
      const verifyPayment = async () => {
        try {
          const res = await fetch(
            `${backend}/api/orders/check-payment?orderId=${orderId}`,
          );
          ...
          if (data.status === "success") {
            ...
            navigate(
              `/payment-failed/${orderId}?message=${encodeURIComponent(msg)}`,
            );
          } else {
          ...
          if (!isMounted) return;
          navigate(
            `/payment-failed/${orderId}?message=${encodeURIComponent("Lỗi kết nối")}`,
          );
        }
      };
