...
export default function WishlistProductCard({ item, onClick, onRemove }) {
  ...
  const {formatPrice } = useCurrency(); // ✅ Hook để định dạng giá
  ...
  const colors = [
    ...new Map(validVariants.filter(v => v.color).map(v => [v.color._id, v.color])).values(),
  ];
  ...
  const allSizes = [
    ...new Map(
      validVariants.map(v => [
        v.size._id,
        { ...v.size, stockQuantity: v.stockQuantity ?? v.stock ?? 0 },
      ])
    ).values(),
  ];
  ...
  return (
    ...
    >
      ...
      <button
        ...
        disabled={removing}
        className={`absolute top-2 right-2 z-10 w-7 h-7 bg-black text-white flex items-center justify-center text-lg font-bold hover:bg-[#ffe6e6] hover:text-black transition-all ${
