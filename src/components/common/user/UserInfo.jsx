...
export default function UserInfo() {
  ...
  const [modalMessage, setModalMessage] = useState(""); // ← STATE CHO MODAL
  ...
  const handleSave = async () => {
    try {
      ...
      // DÙNG MODAL THAY alert()
      setModalMessage(res.message || "Cập nhật thành công!");
      if (res.message?.includes("email") || res.message?.includes("mật khẩu")) {
        setTimeout(() => {
          setModalMessage("Kiểm tra email để xác nhận thay đổi!");
        }, 1500);
      }
      ...
      setModalMessage(err.error || "Lỗi cập nhật thông tin");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <>
      {/* MODAL HIỆN RA KHI CÓ MESSAGE */}
      <AlertModal
        message={modalMessage}
        type={modalMessage.includes("thành công") ? "success" : "error"}
