...
export default function UserInfo() {
  ...
  const handleSave = async () => {
    try {
      ...
      setModalMessage(res.message || t("update_success"));
      if (res.message?.includes("email") || res.message?.includes("mật khẩu")) {
        setTimeout(() => {
          setModalMessage(t("check_email_confirmation"));
        }, 1500);
      }
      ...
      setModalMessage(err.error || t("update_error"));
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="bg-white w-full shadow-lg p-8 mx-auto">
        ...
        {!editing ? (
          <div className="space-y-3 text-gray-700">
            <p>
              <strong>{t("full_name")}:</strong> {user.name || t("not_provided")}
            </p>
            <p>
